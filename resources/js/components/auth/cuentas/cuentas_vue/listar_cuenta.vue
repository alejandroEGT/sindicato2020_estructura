<template>
   <div>
        <br><br>
        <div class="row justify-center">
           <div class="col-md-8">
                <b-card no-body>
                    <b-card-header :style="header_color" >
                    CUENTA
                    </b-card-header>

					        <b-card-body>
                    <div class="row q-col-gutter-md">
                      <div class="col-12 col-md-6">
                        <label for="">Cuenta:</label>
                          <b-form-select 
                            v-model="cuenta_id" 
                            :options="options"
                            value-field="id"
                            text-field="titulo"
                            size="sm"
                            
                            >
                            <template  v-slot:first>
                              <b-form-select-option :value="''">--Seleccione cuenta--</b-form-select-option>
                            </template>

                          </b-form-select>
                      </div>
                      <div class="col-12 col-md-3">
                        <label for="">Año:</label>
                          <b-form-select 
                            v-model="anio" 
                            :options="anios"
                            value-field="id"
                            text-field="label"
                            size="sm"  
                          >
                            <template  v-slot:first>
                              <b-form-select-option :value="''">--Seleccione año--</b-form-select-option>
                            </template>

                          </b-form-select>
                      </div>
                      <div class="col-12 col-md-3">
                        <label for="">Mes:</label>
                          <b-form-select 
                            v-model="mes" 
                            :options="meses"
                            value-field="id"
                            text-field="label"
                            size="sm"  
                          >
                            <template  v-slot:first>
                              <b-form-select-option :value="''">--Seleccione mes--</b-form-select-option>
                            </template>

                          </b-form-select>
                      </div>

                      <div>
                        
                            <b-button variant="dark" size="sm" @click="listar" >
                              <i class="fas fa-search"></i>
                              Buscar</b-button>
                          

                          
                            <b-button variant="success" size="sm" @click="limpiar" >
                              <i class="fas fa-eraser"></i>
                              Limpiar</b-button>
                          

                       
                            <b-button  size="sm" @click="ruta('/modulo-cuentas')">
                              <i class="fas fa-undo-alt"></i>
                              Volver</b-button>
                          
                          
                        
                        <b-button variant="info" size="sm" v-b-modal.modal-1>
                          <i class="far fa-calendar-alt"></i>
                            Inicio mensual
                        </b-button>

                        <b-modal id="modal-1" 
                        hide-footer
                          size="lg"
                          title="Inicio mensual"
                          header-bg-variant="dark"
                          header-text-variant="white">

                          <div>
                              <div>
                                    
                                        <div class="">
                                          <div class="row">
                                            <div class="col-12 col-md-3">
                                              <label for="">Cuenta:</label>
                                              <b-form-select 
                                                v-model="cuenta_id" 
                                                :options="options"
                                                value-field="id"
                                                text-field="titulo"
                                                size="sm"
                                                
                                                >
                                                <template  v-slot:first>
                                                  <b-form-select-option :value="''">--Seleccione cuenta--</b-form-select-option>
                                                </template>

                                              </b-form-select>
                                            </div>
                                            <div class="col-12 col-md-3">
                                              <label for="">Año:</label>
                                              <b-form-select 
                                                v-model="anio" 
                                                :options="anios"
                                                value-field="id"
                                                text-field="label"
                                                size="sm"  
                                              >
                                                <template  v-slot:first>
                                                  <b-form-select-option :value="''">--Seleccione año--</b-form-select-option>
                                                </template>

                                              </b-form-select>
                                            </div>
                                            <div class="col-12 col-md-3">
                                              <label for="">Mes:</label>
                                              <b-form-select 
                                                v-model="mes" 
                                                :options="meses"
                                                value-field="id"
                                                text-field="label"
                                                size="sm"  
                                              >
                                                <template  v-slot:first>
                                                  <b-form-select-option :value="''">--Seleccione mes--</b-form-select-option>
                                                </template>

                                              </b-form-select>
                                            </div>
                                            <div class="col-12 col-md-3">
                                              <label for="">Monto de inicio:</label>
                                              <!-- <q-input v-model="monto_inicio" label="Monto inicial" /> -->
                                               <b-form-input size="sm" v-model="monto_inicio" placeholder="Monto inicial"></b-form-input>
                                            </div>
                                            <!-- <div class="col-12 col-md-2">
                                              <q-btn label="Calcular" />
                                            </div> -->
                                            <!-- <div class="col-12 col-md-2">
                                              <b-button @click="ingresar_inicio_mes">Guardar</b-button>
                                            </div> -->
                                          </div>
                                          <hr>
                                          <b-button
                                            variant="primary"
                                            size="sm"
                                            class="float-right"
                                            @click="ingresar_inicio_mes" 
                                          >
                                            Guardar
                                          </b-button>
                                        </div>


                                  </div>
                          </div>
                        </b-modal>
                      </div>

                    </div>
                    
					        </b-card-body>
                </b-card>
            </div>
        </div>   

         <div v-if="view_tabla">
                <hr>
                <div class="table-responsive" >
                  <table class="table" >
                           <tr :style="header_color">
                             <td>ID</td>
                             <td>SUBCUENTA</td>
                             <td>FECHA</td>
                             <td>ARCHIVO</td>
                             <td>DESCRIPCION descripcion descripcion</td>
                             <td>INGRESO</td>
                             <td>EGRESO</td>
                             <td>OP.</td>
                           </tr>

                            <tr :style="tr_style" v-for="(t,i) in tabla" :key='t.id' >
                               <td :style="border" style="background:#D5DBDB">{{ i+1+'-'+t.id }}</td>
                               <td :style="border">{{ t.titulo }}</td>
                               <td :style="border">{{ t.fecha }}</td>
                               <td :style="border">
                                <b-button size="sm" variant="light" v-b-modal="'graf'+t.id">
                                  <i class="fas fa-file-alt"></i> Archivo</b-button>

                                   <b-modal 
                                   header-bg-variant="dark"
                                   header-text-variant="white"
                                   hide-footer 
                                   size="md" :id="'graf'+t.id" title="Archivo">
                                        <iframe 
                                             style="width:100%; height:500px;" 
                                            :src="'../'+t.archivo" 
                                            frameborder="0" 
                                            allowfullscreen>
                                        </iframe>

                                    </b-modal>
                                 
                               </td>
                               <td :style="border">{{ t.descripcion}}</td>
                               <td :style="border"><label style="color:green">{{formatPrice(t.monto_ingreso)}}</label></td>
                               <td :style="border"><label style="color:red">{{formatPrice(t.monto_egreso)}}</label></td>
                               <td :style="border">
                                 <!-- <q-btn id="show-modal"  @click="show('modal2'+t.id);
                                      llenar_inputs(t.fecha, t.descripcion,t.monto_ingreso,t.monto_egreso)" 
                                color="primary" icon="edit">
                                  <template v-slot:loading>
                                    <q-spinner-facebook />
                                  </template>
                                </q-btn> -->

                                <p><b-button
                                    @click="e_fecha = t.fecha_input; 
                                            e_descripcion=t.descripcion;
                                            e_ingreso=t.monto_ingreso;
                                            e_egreso=t.monto_egreso
                                    "
                                   size="sm" variant="light" v-b-modal="'edit'+i"> Editar</b-button></p>

                                   <b-modal 
                                    header-bg-variant="dark"
                                    header-text-variant="white"
                                    hide-footer 
                                    size="md" 
                                    :id="'edit'+i" 
                                    :title="'Editar ID '+t.id">
            
                                      
                                      <template>
                                          <div class="row justify-center">
                                                <div class="">
                                                  <div class="row q-col-gutter-md">
                                                    <div class="col-12 col-md-12">
                                                        <label for="">Subcuenta</label>
                                                        <b-form-input 
                                                          :value="t.titulo"
                                                          disabled
                                                          size="sm">
                                                        </b-form-input>
                                                    </div>
                                                  </div>
                                                  <br>
                                                  <div class="row q-col-gutter-md">
                                                    <div class="col-10 col-md-10">
                                                     
                                                     <label for="">Fecha</label> 
                                                      <b-form-input
                                                        v-model="e_fecha" 
                                                        
                                                        size="sm" 
                                                        type="date">
                                                      </b-form-input>
                                                    </div>
                                                    <div class="col-2 col-md-2">
                                                        <br>
                                                        <b-button 
                                                        size="sm"
                                                        @click="editar(t.id,'fecha', e_fecha)"
                                                          variant="dark"
                                                        ><i class="fas fa-pen-alt"></i>
                                                        </b-button>
                                                         
                                                    </div>
                                                  </div>
                                                  <br>
                                                  <div class="row">
                                                    <div class="col-10 col-md-10">
                                                      <label for="">Archivo</label>
                                                      <b-form-file id="carga" ref="carga"  @change="arch_carga"  size="sm"  placeholder="seleccione archivo"></b-form-file>
                                                      <!-- <q-input
                                                          @input="val => { file = val[0] }"
                                                              
                                                          type="file"
                                                      /> -->
                                                    </div>
                                                    <div class="col-2 col-md-2">
                                                        <br>
                                                        <b-button 
                                                        size="sm"
                                                        @click="editar_archivo(t.id,'archivo')"
                                                          variant="dark"
                                                        ><i class="fas fa-pen-alt"></i>
                                                        </b-button>
                                                    </div>
                                                  </div>
                                                  <br>
                                                  <div class="row q-col-gutter-md">
                                                    <div class="col-10 col-md-10">
                                                      <label for="">Descripción</label>
                                                      <b-form-textarea  
                                                          v-model="e_descripcion"
                                                          placeholder="Modificar..."
                                                          rows="3"
                                                          max-rows="6"
                                                        ></b-form-textarea>
                                                    </div>
                                                    <div class="col-2 col-md-2">
                                                        <br>
                                                        <b-button 
                                                        size="sm"
                                                        @click="editar(t.id,'descripcion', e_descripcion)"
                                                          variant="dark"
                                                        ><i class="fas fa-pen-alt"></i>
                                                        </b-button>

                                                    
                                                    </div>
                                                  </div>
                                                  <br>

                                                
                                                  <div class="row q-col-gutter-md">
                                                    <div class="col-10 col-md-10">
                                                      <label for="">Ingreso</label>
                                                      <b-form-input
                                                        size="sm"
                                                        v-model="e_ingreso"
                                                        type="number"
                                                      ></b-form-input>
                                                    </div>
                                                    <div class="col-2 col-md-2">
                                                      <br>
                                                        <b-button 
                                                        size="sm"
                                                        @click="editar(t.id,'ingreso', e_ingreso)"
                                                          variant="dark"
                                                        ><i class="fas fa-pen-alt"></i>
                                                        </b-button>
                                                    </div>
                                                  </div>

                                                  <br>
                                                  <div class="row q-col-gutter-md">
                                                    <div class="col-10 col-md-10">
                                                      <label for="">Egreso</label>
                                                      <b-form-input
                                                        size="sm"
                                                        v-model="e_egreso"
                                                        type="number"
                                                      ></b-form-input>
                                                    </div>
                                                    <div class="col-2 col-md-2">
                                                        <br>
                                                        <b-button 
                                                        size="sm"
                                                        @click="editar(t.id,'egreso', e_egreso)"
                                                          variant="dark"
                                                        ><i class="fas fa-pen-alt"></i>
                                                        </b-button>
                                                    </div>
                                                  </div>


                                                  <hr>
                                                  <div class="row q-col-gutter-md">
                                                    <div class="col-8">
                                                      <br>
                                                      <b-button 
                                                      @click="eliminar(t.id, 'edit'+t.id)"
                                                      variant="danger">
                                                      <i class="fas fa-trash-alt"></i> Eliminar
                                                      </b-button>
                                                      
                                              
                                                    </div>

                                                  
                                                </div>
                                                  
                                          
                                              </div>

                                          </div>
                                        </template>
                                  
                                    </b-modal>
                       
                               </td>
                            </tr>
                            

                            <tr >
                             <th :style="header_color" colspan="5">RESUMEN DEL MES</th>
                             <td :style="border" >{{data_resumen[0].valor}}</td>
                             <td :style="border" >{{data_resumen[1].valor}}</td>
                             <td :style="border" ><b>{{data_resumen[2].valor}}</b></td>
                           </tr>

                           <tr >
                             <th :style="header_color" colspan="3">ACUMULADO MESES ANTERIORES</th>
                             <td :style="border" colspan="1">{{formatPrice(monto_inicio)}}</td>
                             <th :style="header_color" colspan="3">ARRASTRE TOTAL</th>
                             <td :style="border">{{formatPrice(monto_inicio +(ingresos-egresos))}}</td>
                           </tr>
                  
                           

                    </table>
                </div>


                
                
              </div>
            

            <hr>
            <div class="col-6">

                     
                         
              <template>
						  <div class="q-pa-md">
                

						   
						  <q-list>
						    <q-item v-for="s in selected" :key="s.id">
						      {{s.name}}
						    </q-item>                 
						  </q-list>
                <br>
                
						    <!-- <div class="q-mt-md">
						      Selected: {{ JSON.stringify(selected) }}
						    </div> -->
						  </div>
              </template>
                  

            <!-- {{ ingresos +' - '+egresos + ' = '+ (ingresos-egresos) }} -->
            
      
            </div>
            
   </div>
   
    
</template>  

<script src="../cuentas_js/listar_cuentas.js"></script> 